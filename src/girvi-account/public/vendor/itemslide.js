...
$(function() { //document ready
    ...
    $.fn.itemslide = function(options) {
            ...
            //move to start slide without animation
            gotoSlideWithoutAnimation(settings.start);
                    ...
                    if (isDown) {
                        ...
                        if (vertical_pan) {
                            ...
                            if ((touch.pageX - startPointX) * direction < 6 * (-1)) { //Check distance to see if the event is a tap
                                var index = getLandingSlideIndex(vars.velocity * settings.swipe_sensitivity - slides.translate3d().x);
                                if (index != vars.currentIndex) { //Check if landingIndex is different from currentIndex 
                                    slides.trigger("animationStart", direction);
                                }
            ...
            function isOutBoundaries() { //Return if user is panning out of boundaries
                return (((Math.floor(slides.translate3d().x) > (getPositionByIndex(0)) && direction == -1) || (Math.ceil(slides.translate3d().x) < (
                    getPositionByIndex(slides.children('li').length - 1)) && direction == 1)));
            }

            function gotoSlideByIndex(i) {
                slides.jDeferred = $.Deferred();
                ...
                return slides.jDeferred.promise();
            ...
            function animationRepeat() { //epeats using requestAnimationFrame //For the sliding
            ...
            function gotoSlideWithoutAnimation(i) {
                changeActiveSlideTo(i);
                vars.currentIndex = i;
                slides.currentLandPos = getPositionByIndex(i);
                slides.translate3d(slides.currentLandPos);
            ...
            slides.gotoSlide = function(i) {
                return gotoSlideByIndex(i);
            };

            slides.next = function() {
                return gotoSlideByIndex(vars.currentIndex + 1);
            };
    ...
    $.fn.translate3d = function(x, y) {
        if (x != null) { //Set value
            ...
            if ($.fn.jquery != null) { //This happens if has jQuery
                ...
                var vals = this.css('transform').replace("translate3d", "").replace("(", "").replace(")", "").replace(" ", "").replace("px", "").split(
                    ","); //Consider regex instead of tons of replaces
